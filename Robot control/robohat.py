...
# spinLeft(speed): Sets both motors to move forward at speed. 0 <= speed <= 100
def spinLeft(speed):
...
# spinRight(speed): Sets both motors to reverse at speed. 0 <= speed <= 100
def spinRight(speed):
    p.ChangeDutyCycle(0)
...
# forward(speed): Sets motors to turn opposite directions at speed. 0 <= speed <= 100
def forward(speed):
    p.ChangeDutyCycle(0)
    q.ChangeDutyCycle(speed)
    a.ChangeDutyCycle(speed)
...
# reverse(speed): Sets motors to turn opposite directions at speed. 0 <= speed <= 100
def reverse(speed):
    p.ChangeDutyCycle(speed)
    q.ChangeDutyCycle(0)
    a.ChangeDutyCycle(0)
...
def set20to20motorsSpeed(leftSpeed,rightSpeed):

    if leftSpeed>10:
       realspeed=(leftSpeed-10)*10
       p.ChangeDutyCycle(realspeed)
#       q.ChangeDutyCycle(0)
       p.ChangeFrequency(realspeed + 5)
    else:
       realspeed=leftSpeed*10
#       p.ChangeDutyCycle(0)
       q.ChangeDutyCycle(realspeed)
       q.ChangeFrequency(realspeed + 5)

    if rightSpeed>10:
       realspeed=(rightSpeed-10)*10
       a.ChangeDutyCycle(realspeed)
 #      b.ChangeDutyCycle(0)
       a.ChangeFrequency(realspeed + 5)
    else:
       realspeed=rightSpeed*10
#       a.ChangeDutyCycle(0)
       b.ChangeDutyCycle(realspeed)
       b.ChangeFrequency(realspeed + 5)
...
def wheelCount():
    ...
    lastValidL = 2
    lastValidR = 2
    lastL = GPIO.input(wheelLeft)
    lastR = GPIO.input(wheelRight)
    while running:
        time.sleep(0.002)
        val = GPIO.input(wheelLeft)
        if val == lastL and val != lastValidL:
            countL += 1
            lastValidL = val
        lastL = val
        val = GPIO.input(wheelRight)
...
def initWheelValues():
    global running, countL, countR ,lastL ,lastR, lastValidL , lastValidR
    lastValidL = 2
    lastValidR = 2
    GPIO.setup(wheelLeft, GPIO.IN)
    lastL = GPIO.input(wheelLeft)
    GPIO.setup(wheelRight, GPIO.IN)
    lastR = GPIO.input(wheelRight)

    
def updateWheelValues():
    global running, countL, countR ,lastL , lastValidL , lastValidR , lastR
...
# stepForward(speed, steps): Moves forward specified number of counts, then stops
def stepForward2(speed, counts,qstopRequest,qsendCounts,qdata):
    global countL, countR 
    countL = 0
    countR = 0
    runL = True
    ...
    while runL or runR:
        print ("fooorward")
        updateWheelValues()
        qsendCounts.put([countL,countR])

        print ("fooorward4")
        if qstopRequest.qsize()==0:
            
            time.sleep(0.002)
            if countL >= counts and counts!=0:
                stopL()
                runL = False
            if countR >= counts and counts!=0:
                stopR()
                runR = False
        else:
            print ("stoped")
            var= qstopRequest.get()
            stopR()
            stopL()
            while qsendCounts.qsize()>0:
                qsendCounts.get()
            if var[1]==0:
                while qdata.qsize()>0:
                    qdata.get()
...
# stepReverse(speed, steps): Moves backward specified number of counts, then stops
def stepReverse2(speed, counts,qstopRequest,qsendCounts,qdata):
    global countL, countR
    countL = 0
    countR = 0
    runL = True
...
# stepSpinL(speed, steps): Spins left specified number of counts, then stops
def stepSpinL2(speed, counts,qstopRequest,qsendCounts,qdata):
    global countL, countR
    countL = 0
    countR = 0
    initWheelValues()
    spinLeft(speed)
    while countL<counts or countR<counts or counts==0:
        updateWheelValues()
...
# stepSpinR(speed, steps): Spins right specified number of counts, then stops
def stepSpinR2(speed, counts,qstopRequest,qsendCounts,qdata):
    global countL, countR
    countL = 0
    countR = 0
    initWheelValues()
